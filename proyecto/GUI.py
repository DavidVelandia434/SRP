...
title_label_widget = tk.Label(
    left_info_frame, text="", font=("Segoe UI", 16, "bold"),
    bg=BG_PANEL, fg=ACCENT
)
director_label = tk.Label(
    left_info_frame, text="", bg=BG_PANEL,
...
poster_link = tk.Label(
    left_info_frame, text="", fg=ACCENT, cursor="hand2",
    bg=BG_PANEL, font=("Segoe UI", 10, "underline")
)

# Derecha
overview_label = tk.Label(
    right_info_frame, text="", wraplength=700, justify="left",
...
def confirm_selection():
    ...
    movies = DataBase.get_qualified_movies_by_rate(
        DataBase.filter_by_genres(selected_genres)
    )

    for i, m in enumerate(movies):
        if m["title"] not in selected_movies:
            tag = "evenrow" if i % 2 == 0 else "oddrow"
            tree.insert(
                "", "end",
                values=(m["id"], m["title"], m["vote_average"]),
                tags=(tag,)
            )
...
def show_movie_details(event=None):
    ...
    if "error" in details:
        ...
        for lbl in [
            director_label, runtime_label, genres_label,
            rating_label, overview_label, poster_link
        ]:
            lbl.config(text="")
        return
...
# ---------------------------- CABECERA ----------------------------
tk.Label(
    root, text="🎬 Movie Recomendation System",
    font=("Segoe UI", 20, "bold"), bg=BG_MAIN, fg=ACCENT
).pack(pady=15)

tk.Label(
    root, text="Select one or more genres:",
    font=("Segoe UI", 13, "bold"), bg=BG_MAIN, fg=TEXT_MAIN
).pack()
...
# ---------------------------- BOTÓN ----------------------------
def on_enter(e):
    filter_button.config(bg=ACCENT, fg=BG_MAIN)
